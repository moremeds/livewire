...
def get(path, **q):
    ...
    with urllib.request.urlopen(
        f"https://api.eia.gov/v2/{path}?" + urllib.parse.urlencode(q, doseq=True), timeout=90
    ) as r:
...
def walk(path):
    try:
        ...
        emit({"route": path, "error": str(e)})
        print(path, "ERR", e, file=sys.stderr)
        return
    if r.get("routes"):
        for x in r["routes"]:
            walk(f"{path}{x['id']}/")
    ...
    try:
        ...
        leaf = {
            ...
            "data": {
                k: ((v.get("alias"), v.get("units")) if isinstance(v, dict) else v)
        ...
        emit({"route": path, "error": f"{type(e).__name__}: {e}"})
        print(path, "ERR", e, file=sys.stderr)
