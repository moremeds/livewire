...
CONS=[("CLX6",304037511),("RBX6",600696516),("ZSX6",597391703)]
try:
    client.connect(host="127.0.0.1",port=4001,client_id=7,timeout=10)
    client.ib.errorEvent += col
    for name,cid in CONS:
        c=Contract(conId=cid); client.ib.qualifyContracts(c)
        for label,dur,bs,end in (("1h@2022-01","1 M","1 hour","20220115-00:00:00"),
                                 ("1m@2024-06","1 D","1 min","20240615-00:00:00")):
            try:
                bars=client.get_historical_data(c,duration=dur,bar_size=bs,what_to_show="TRADES",use_rth=True,end_date=end)
                out["probes"].append({"contract":name,"req":label,"rows":len(bars),
                    "first":str(bars[0].date) if bars else None,
                    "last":str(bars[-1].date) if bars else None,"errors":errs[:]})
            except Exception as e:
                out["probes"].append({"contract":name,"req":label,"exc":repr(e)[:200],"errors":errs[:]})
