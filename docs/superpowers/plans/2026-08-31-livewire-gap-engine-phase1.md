...
5. **L4a — delisted membership.** Archived symbols leave the live denominator. Production holds 8,620 of them against a 14,811-symbol live universe (spec §4.3), so this is a directory listing with no provider and no credential. Ship it first; it is the half of L4 that is cheap and unblocked.
5b. **L4b — delisting terminus.** Give those 8,620 a `delisted_at` and separate rename from delisting. `universe_sync.py:273` is the only `mark_delisted` caller and has never run in production; `registry.json` does not exist. Run it, then adjudicate the residue — at this population size that is the agent lane (spec §9.3), not manual review.
