...
5. **L4a — reconcile the 234 preset/archive conflicts.** 234 of the 8,620 archived symbols are still claimed by a preset (BK, 63 ADRs, 157 ETFs); `KALV` and `MAPS` are in both trees at once. Per symbol one of the two universes is wrong. This replaces the "archived symbols leave the denominator" idea, which spec §4.3 now rejects outright — it would have hidden BK's live G3 hole. First batch for the agent lane.
6. **L4b — delisting terminus.** Give those 8,620 a `delisted_at` and separate rename from delisting. `universe_sync.py:273` is the only `mark_delisted` caller and has never run in production; `registry.json` does not exist. Run it, then adjudicate the residue — at this population size that is the agent lane (spec §9.3), not manual review.
