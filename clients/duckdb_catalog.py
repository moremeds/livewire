...
def build_coverage(
    ...
    *,
    ...
    absent on a fresh lake. A source that exists but cannot be read aborts the
    build, and the build also fails if *every* source is empty.
    ...
    try:
        ...
        for view_name, date_column in sources:
            try:
                ...
                if not str(exc).startswith("IO Error: No files found that match the pattern "):
                    raise
                spec = view_spec(view_name, lake_root=lake_root, silver_root=silver_root)
                try:
                    with os.scandir(spec.directory) as partitions:
                        for partition in partitions:
                            if not partition.is_dir():
                                continue
                            try:
                                (Path(partition.path) / spec.filename).stat()
