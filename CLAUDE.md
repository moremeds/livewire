...
- Six launchd jobs on the mini: intraday-catchup 05:00Z → daily-update 06:00Z (4h total deadline) → watchdog 10:30Z → coverage 11:00Z (no timeout) → release-promote; universe-refresh weekly, from the repo.
...
- `MDW_DAILY_JOB_DEADLINE_SECONDS` (4h) is a **total** across seven sequential lanes; a lane that exhausts it is killed by process group, never retried, and pages. (2026-08-29 and 09-01: corporate-actions alone ran 8h39m / 3h57m, so equity was killed and Silver skipped — the CA slowdown is unexplained and open.) → test: `tests/test_run_daily_update_job.py::test_a_lane_started_past_the_deadline_never_runs` · pm:2026-07-28-daily-job-deadline-is-a-total
...
- coverage/weekly/digest run once, after Silver. The watchdog requires the `silver` scope and treats an absent quality marker as failure only after `=== Job complete <ts> ===`. → test: `tests/test_check_daily_update_watchdog.py` · pm:2026-07-22-coverage-weekly-digest-ordering, pm:2026-08-16-watchdog-raced-quality-marker
...
- Undelivered alerts persist to `<log_dir>/alerts_undelivered/` and `MDW_UNDELIVERED_DIR`; `status` and the watchdog count both. A WARNING in a log nobody reads is not an alert. → test: `tests/test_status.py::test_any_undelivered_alert_is_a_warning`
...
- `status`: `UNKNOWN` is not `OK` (`Verdict` is an `IntEnum`, OK < UNKNOWN < WARN < BAD); a missing log is BAD, a log with no `SUMMARY_JSON` is UNKNOWN; coverage reads the **last** `coverage:` line; `launchctl` exits cap at WARN; Silver is graded on the change in `failed`; every log line goes through `rich.markup.escape`; exit code is always 0; it never scans parquet. → test: `tests/test_status.py` · pm:2026-08-16-status-surface-grading, pm:2026-08-14-coverage-log-first-line-oldest
