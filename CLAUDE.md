...
- A run opens through `ledger.open_run`, which closes its own job's still-open predecessors on this host as `ABANDONED` (carrying their `started`, so a real terminal row still wins on `max(ended)`): a dead process cannot write its own close row, and no reader may be asked to infer "no close row and no process". Entrypoints catch `BaseException` around the body, because Ctrl-C is not an `Exception`. → test: `tests/test_ledger.py::test_open_run_closes_a_predecessor_that_never_wrote_its_own_terminal_row`, `::test_a_late_real_close_row_outranks_the_abandoned_one` · pm:2026-09-16-interrupted-runs-never-closed
- A failure measurement names its subject, not just its count: `identity_fetch_failed` told the operator to rerun without saying what, so the only recovery was the whole universe. Each failure files `identity_fetch_failed_ticker`, scope `<ticker>:<http status>`. → test: `tests/test_security_master_sync.py::test_a_fetch_failure_names_the_ticker_and_its_http_status` · pm:2026-09-16-fetch-failures-had-no-subject
...
- An agent repair that does write the lake goes through an entrypoint that emits `runs` + `measurements`, or it did not happen: 39 dividend rows were superseded by hand with no ledger row, invisible to `status`/digest, and one provider-scoped reconcile from being reverted unseen (a non-`massive` active head is now the current answer). → test: `tests/test_corporate_action_store.py::test_full_reconcile_after_conversion_leaves_the_eod_fx_row_active` · pm:2026-09-13-corporate-actions-mutated-outside-the-ledger
...
- A PIT action receipt (v2) proves each event's head at as-of against the latest verified page — id + payload hash, or a split's date + ratio (Massive re-ids splits) — and never copies the store's live `status`, which `reconcile` rewrites in place: v1 demanded provenance on July rows that can never get it (43/458 sp500) and stopped replaying when AXON was revised two hours after publish. A v1 receipt is refused, not replayed. → test: `tests/test_shepherd_actions.py::test_a_provider_revision_after_as_of_does_not_change_the_replay` · pm:2026-09-23-pit-action-receipt-proved-lineage-not-the-answer
...
- A rate constant is the provider's published limit for _that_ plan and endpoint, or it is measured — never the only number already lying around: the FX free tier's 5/min was seeded onto paid `/reference` and paced the identity backfill at 12 s/request, 40+ hours of pure sleep (Massive paid is uncapped, advised < 100 req/s; declared 600/min, latency-bound at ~24 tickers/min). → test: `tests/test_constants.py::test_the_reference_endpoint_rate_limit_is_scoped_like_the_fx_one` · pm:2026-09-16-reference-rate-inherited-the-free-fx-tier
- A stale non-equity symbol is a claim about the provider, not about livewire: query that symbol's own last observation upstream before touching a lane. CBOE retired `VIXTLT` after 2026-07-30 and livewire held every one of its 2,150 rows, yet coverage mailed a failure nightly for five weeks. → pm:2026-09-07-retired-cboe-index-alerted-forever
- FRED answers a good request with a 502 or a read timeout now and then: the client retries 5xx/timeouts (`fred_retry_attempts`), never a 4xx, and one series' failure never skips the rest — a still-unfetched series fails the phase, so an outage never reads as `inserted 0`, exit 0. Rates ride **only** on intraday-catchup phase 2; there is no rates lane in daily-update. → test: `tests/test_fred_client.py::test_a_502_is_retried_and_the_next_attempt_is_used`, `tests/test_fetch_fred_rates.py::test_a_total_outage_is_a_failure_not_an_empty_success` · pm:2026-09-14-fred-502-aborted-remaining-series
- A transient HTTP failure is `clients/http_retry.py` and nowhere else: 5xx and transport errors retry, a 4xx never does. Per-symbol isolation continues the _other_ symbols, it never makes the phase exit 0 — `fetch_cboe_volatility` swallowed every exception and returned `None`, so the CBOE phase could not fail however much was missing; it now returns 1 for anything still unfetched after retries, and 0 for a 4xx, which is CBOE retiring an index, not a livewire failure. → test: `tests/test_fetch_cboe_volatility.py::TestMainExitCode::test_returns_1_when_a_symbol_is_still_unfetched_after_retries`, `tests/test_http_retry.py` · pm:2026-09-16-cboe-could-not-fail
...
- `gap_engine.actual_sessions` is the fourth reader of the same file class: an unreadable parquet is an empty set, so the series reads `G3` and enters the queue — raising it degraded the whole gap scan to one `scan: FAILED` line while the rest of coverage looked healthy. → test: `tests/test_gap_engine.py::test_one_corrupt_parquet_is_counted_missing_and_does_not_kill_the_scan` · pm:2026-09-07-gap-engine-aborted-on-a-corrupt-parquet
- `duckdb build` aborts on unreadable sources and preserves the previous catalog; only a genuine no-files match is optional. Skipping corrupt RJF published `bronze_equity_1d: 0` with exit 0. Status grades missing expected views BAD. → test: `tests/test_duckdb_catalog.py::test_build_coverage_rejects_corrupt_parquet_without_publishing`, `tests/test_status.py::test_one_missing_catalog_view_cannot_be_hidden_by_current_other_views` · pm:2026-09-08-partial-catalog-reported-success
...
- A lake sweep walks with `os.walk(followlinks=True)`, never `rglob` (it will not enter a symlinked directory), and a first-level subtree the walk never entered is a WARNING, not a clean summary: since 2026-09-06 `bronze/`…`security_master/` are symlinks onto the volume and the sweep deleted 71 of 324k sidecars, exit 0. → test: `tests/test_housekeeping.py::TestTheSweepCrossesTheSymlinkedSubtrees` · pm:2026-09-07-appledouble-sweep-stopped-at-the-symlinked-subtrees
- Anything that enumerates files on the exFAT lake filters `._` AppleDouble sidecars: the raw staging dir is created inside the raw root, so `glob("*.parquet")` matched `._bucket=000.parquet` and one un-decodable 4096-byte sidecar killed every Massive raw publication (2026-09-09; intraday-catchup + the 14,837-symbol 1m republish). Readers glob `bucket=*.parquet`, which cannot match. → test: `tests/test_massive_raw_publication.py::test_stage_validation_ignores_the_appledouble_sidecars_of_the_exfat_lake` · pm:2026-09-09-raw-stage-validator-read-the-appledouble-sidecar
