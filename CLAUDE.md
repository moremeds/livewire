...
- **`status.py` must never `import duckdb`** — it reads the catalog through
  `clients.duckdb_catalog.coverage_headline`, which is also where DuckDB's
  `CatalogException` is translated to `FileNotFoundError` (the caller cannot
  catch it without the import `tests/test_duckdb_containment.py` forbids). The
  import is guarded so the command still runs in `~/market-warehouse/.venv`,
  which has no duckdb installed.
