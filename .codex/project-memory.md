...
- BZ is retired from new futures ingestion by user decision. Daily futures discovery skips existing BZ contract directories, explicit BZ ingestion requests are rejected, and historical BZ parquet remains readable under BZ. COIL is a separate IPE-root series; never rename or concatenate BZ history into it. The scheduled futures daily lane resolves `presets/futures-rolling.json` against IB ContractDetails, selects energy contracts through the current month plus 15 months and the first two listed metals/agriculture contracts, and full-history seeds newly entering contracts before daily updates.
