...
- `scripts/livewire_ingest.py corporate-actions` reconciles Massive splits and cash dividends into `data-lake/bronze/asset_class=corporate_action/symbol=<encoded_symbol>/events.parquet`. Provider corrections and cancellations are retained as linked event revisions; cancellation inference requires `--full-reconcile`, and automated scheduling belongs to the Silver-engine phase so reconciliation precedes adjusted-bar publication.
