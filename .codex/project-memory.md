...
- `scripts/livewire_ingest.py corporate-actions` reconciles Massive splits and cash dividends into `data-lake/bronze/asset_class=corporate_action/symbol=<encoded_symbol>/events.parquet`. Provider corrections and cancellations are retained as linked event revisions; cancellation inference requires `--full-reconcile`.
...
- `scripts/livewire_quality.py validate-adjusted-history` is the strict read-only full-equity-history gate. It uses Massive adjusted bars first, fills unavailable dates with freshly normalized IB history, validates pointwise OHLC plus 20/50/200-session averages and Silver total-return reconstruction, and cannot pass with unresolved local dates. Its external output caches and cursor are bound to canonical input hashes; IB replay evidence is explicitly not labelled independent vendor validation.
