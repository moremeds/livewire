...
- Split-basis classification ignores actions at or before the first stored session because no stored row can be affected. Actions after the last stored session remain pending unless repeated provider evidence includes a post-event bar and the stored pre-event scale decisively matches raw or adjusted treatment; the partial post-event price itself is not used. Ambiguous in-history boundaries are resolved only from repeated pointwise-consistent IB windows. Because IB history may itself be raw or adjusted, its basis is first classified at the action boundary; repeated Massive-adjusted ranges are a narrow fallback. The same evidence path may recover nonpositive OHLC fields in their existing basis. The audit replays saved provider rows against current Bronze/action inputs instead of trusting a saved label.
- Daily and intraday bronze replace/merge operations use blocking `fcntl.flock`
  ...
  data lake.
- Write coordination stays narrow: Silver takes exclusive equity/action input
  locks only while copying its source snapshot, Bronze writers hold compatible
  shared input locks plus their per-symbol locks, and the Silver publisher
  serializes only the revision transaction. There is no runner-wide lake lock.
...
- Every email is an `executions(script='notify')` row — success, failure, and dedup skip. `livewire_node/send_mail.mjs` is SMTP-only (Resend: `smtp.resend.com:465`, From `Livewire <livewire@rsiarc.com>`); `notify.py` is the sole email surface. Credentials live in warehouse `.env` on the mini, never in a release.
...
- BZ is retired from new futures ingestion by user decision. Daily futures discovery skips existing BZ contract directories, explicit BZ ingestion requests are rejected, and historical BZ parquet remains readable under BZ. COIL is a separate IPE-root series; never rename or concatenate BZ history into it. The scheduled futures lane resolves `presets/futures-rolling.json` against IB ContractDetails, full-history seeds newly selected contracts, then scans all existing non-retired futures directories. Coverage uses the same live IB selection for its futures denominator; IB failure marks futures UNKNOWN without stopping equity coverage or recovery.
...
- `scripts/livewire_ingest.py corporate-actions` reconciles Massive splits and cash dividends into `data-lake/bronze/asset_class=corporate_action/symbol=<encoded_symbol>/events.parquet`. Provider fetches default to four independent worker sessions, while canonical store and scope-specific atomic cursor writes remain serialized. Corporate-action identity must preserve provider-significant mixed case (`BCPC`/`BCpC`, `TPC`/`TpC`); fully lowercase user input may normalize to uppercase, but mixed-case provider symbols must never be collapsed. `--resume` continues only an incomplete compatible cursor; successful reconciliation checkpoints the symbol, and cancellation inference still requires `--full-reconcile`.
- `scripts/livewire_store.py rebuild-silver` derives fully back-adjusted daily bars plus compact factor intervals under immutable `data-lake/silver/generations/<attempt-id>/` paths without mutating bronze. Splits adjust price and volume; cash dividends adjust price only. `revisions/current.json` is the only commit record and is replaced last; compliant readers resolve only its referenced artifact paths and hashes. An immutable manifest ahead of current is uncommitted and is quarantined on retry, while its generation is retained. DuckDB uses `SilverSnapshot` to bind one manifest per connection. The producer-to-Apex adapter contract is `docs/plans/2026-09-08-silver-atomic-publication.md`.
...
- A full flat-file backfill is capacity-gated before download using the discovered compressed object size, `MDW_FLATFILE_STORAGE_MULTIPLIER`, and `LW_DECLARED_FLATFILE_MIN_FREE_GB`.
