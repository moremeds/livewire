...
- `scripts/livewire_ingest.py daily-backfill` is the lightweight routine catch-up runner for an already-seeded warehouse. It uses Massive for recent equity daily repair and recent-window equity intraday, keeps FRED/CBOE/IB volatility side lanes on their existing sources, and honors `MDW_DAILY_BACKFILL_INTRADAY_DAYS` (default 7) for the intraday lookback window.
