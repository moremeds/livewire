...
- `membership-sync repair-identity`: merges a `massive` identity into its researched (`wikipedia_sec_research`) twin by CIK (R1), extends a researched claim's narrow window to the next non-churn remove (R2), and rejects same-timestamp remove+add churn pairs (R4) — fixes the 2026-09-17 narrow-identity-window membership churn. Dry run by default; `--apply` writes a JSON manifest and a ledger run. The live diff (`membership-sync`) now also compares tickers rather than `security_id`s, so a narrow researched window no longer churns membership on its own.
